...
class S2P2(TorchBaseModel):
    def __init__(self, model_config):
        ...
        # Move all layers to device (needed because layers are created after super().__init__)
        self.to(self.device)

    def to(self, *args, **kwargs):
        """Override to() to update event_sampler device for multi-GPU compatibility."""
        ...
        if args:
            ...
            if isinstance(first_arg, torch.device):
                device = first_arg
            elif isinstance(first_arg, str) and ('cuda' in first_arg or 'cpu' in first_arg):
                device = torch.device(first_arg)
            elif isinstance(first_arg, int):
                device = torch.device(f'cuda:{first_arg}' if first_arg >= 0 else 'cpu')
        if 'device' in kwargs:
            device = kwargs['device']
            if isinstance(device, str):
                device = torch.device(device)

        # Update self.device and event_sampler.device
        if device is not None:
            self.device = device
            if self.event_sampler is not None:
                self.event_sampler.device = device

        return result

    def cuda(self, device=None):
        """Override cuda() to update event_sampler device."""
        ...
        if self.event_sampler is not None:
            self.event_sampler.device = self.device
        return result

    def make_dtime_loss_samples(self, time_delta_seq):
        """Generate the time point samples for every interval.

        Override base method to use input tensor's device for multi-GPU compatibility.
        ...
        """
        # Use the device of the input tensor for multi-GPU compatibility
    ...
    def loglike_loss(self, batch, **kwargs):
        ...
        # If hybrid loss is enabled, compute MSE and combine with uncertainty weighting
            ...
            # Compute NLL loss (using weighted formula for hybrid mode)
    ...
    def predict_one_step_at_every_event(self, batch):
        ...
        Override to sync event_sampler device with input batch for multi-GPU compatibility.
        """
        # Sync event_sampler device with input data device
        time_seq = batch[0]
        if self.event_sampler is not None and time_seq.device != self.event_sampler.device:
            self.event_sampler.device = time_seq.device

        # Call parent implementation
        return super().predict_one_step_at_every_event(batch)
