...
class ODETPP(TorchBaseModel):
    ...
    def __init__(self, model_config):
        ...
        # Time normalization factor to prevent numerical instability
        # Scale down large time values for stable ODE integration
        self.time_factor = model_config.model_specs.get('time_factor', 0.001)
    ...
    def forward(self, time_delta_seqs, type_seqs):
        ...
        # Scale down time deltas for numerical stability
        time_delta_seqs_scaled = time_delta_seqs * self.time_factor
        time_delta_seqs_ = time_delta_seqs_scaled[..., None]
        ...
        for type_emb, dt in zip(torch.unbind(type_seq_emb, dim=-2),
            ...
            # ODE integration with scaled time
    ...
    def loglike_loss(self, batch):
        ...
        # If event-only loss is enabled (event NLL + MSE with uncertainty weighting)
            ...
            total_intensity = lambda_at_event.sum(dim=-1)  # [batch_size, seq_len]
    ...
    def compute_states_at_sample_times(self, state_ti_plus, sample_dtimes):
        ...
        # Scale sample times for numerical stability
        sample_dtimes_scaled = sample_dtimes * self.time_factor
        ...
        state = self.solver(diff_func=self.event_model,
                            dt=sample_dtimes_scaled[..., None],  # [batch_size, seq_len, num_samples, 1]
